  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Control what each role can access across the admin panel."
        actions={
            ...
            <TableBody>
              {roles.map((role) => (
                <TableRow key={role._id}>
                  ...
                  <TableCell className="text-sm">{role.permissions?.length ?? 0} granted</TableCell>
                  <TableCell>
...
function RoleFormDialog({
  ...
  const toggleAll = (permNames: string[]) => {
    setSelected((prev) => {
      const next = new Set(prev);
      const allSelected = permNames.every((p) => next.has(p));
      permNames.forEach((p) => (allSelected ? next.delete(p) : next.add(p)));
      return next;
    });
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>Define the role and its permissions.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-4 rounded-xl border p-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold">Permissions ({selected.size})</p>
            ...
            {Object.entries(grouped).map(([group, perms]) => (
              <div key={group} className="rounded-lg border p-3">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <Button type="button" variant="ghost" size="sm" className="h-6 text-xs" onClick={() => toggleAll(perms.map((p) => p.name))}>
                    Toggle all
                  </Button>
                </div>
